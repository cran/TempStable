...
GMMParametersEstim_GTS <-
  function(x,
    ...
    Estim <- switch(algo,
                    `2SGMM` = {
                      Compute2SGMMParametersEstim_GTS(
                        x = x,
                        theta0 = theta0,
                        alphaReg = alphaReg,
                        regularization = regularization,
                    ...
                    ITGMM = {
                      ComputeITGMMParametersEstim_GTS(
                        x = x,
                        theta0 = theta0,
                        alphaReg = alphaReg,
                        regularization = regularization,
                    ...
                    CueGMM = {
                      ComputeCueGMMParametersEstim_GTS(
                        x = x,
                        theta0 = theta0,
                        alphaReg = alphaReg,
                        regularization = regularization,
    ...
    if (PrintTime) {
      CallingFct <-
        paste("GTS", "GMMParametersEstim", algo, t_scheme, sep = "_")
...
Compute2SGMMParametersEstim_GTS <-
  function(x,
    ...
    ProvidedWeightingMatrix <-
      ComputeWeightingMatrix_GTS(t, theta1, x, WeightingMatrix, ...)
    CurrentEstimOptInfo <-
      ComputeCurrentEstim_GTS(
        t_scheme = t_scheme,
        theta0 = theta1,
...
ComputeCurrentEstim_GTS <-
  function(t_scheme,
    ...
    optOutput <-
      stats::nlminb(
        start = theta0,
        objective = ComputeObjective_GTS,
        gradient = NULL,
        hessian = NULL,
        t = t,
        x = x,
...
ComputeObjective_GTS <-
  function(theta,
    ...
    gbar <- sampleRealCFMoment_GTS(x = x, t = t, theta = theta)
    K1gbar <-
      ComputeInvKbyG_GTS(
        K = K,
        G = gbar,
...
sampleRealCFMoment_GTS <- function(x, t, theta) {
  ComplexRes <- sampleComplexCFMoment_GTS(x, t, theta)
  return(c(Re(ComplexRes), Im(ComplexRes)))
}

sampleComplexCFMoment_GTS <- function(x, t, theta) {
  ecf <- function(tt)
    mean(exp(complex(imaginary = tt * x)))
  phiXn <- sapply(t, ecf)
  phiTheta <- ComplexCF_GTS(t, theta)
  return(phiXn - phiTheta)
}

ComplexCF_GTS <- function(t, theta) {
  alphap <- theta[1]
  alpham <- theta[2]
  deltap <- theta[3]
  deltam <- theta[4]
  ...
  lambdam <- theta[6]
  mu <- theta[7]
  CheckParametersRange_GTS(c(alphap, alpham, deltap, deltam, lambdap, lambdam, mu))
  charGTS(t, alphap, alpham, deltap, deltam, lambdap, lambdam, mu)
}
...
ComputeWeightingMatrix_GTS <-
  function(t, theta, x, WeightingMatrix, ...) {
    switch(
      WeightingMatrix,
      OptAsym = {
        K <- asymVarRealCFMoment_GTS(t = t, theta = theta)
      },
      DataVar = {
        K <- DataVarRealCFMoment_GTS(t = t, theta = theta, x = x)
      },
      Id = {
...
asymVarRealCFMoment_GTS <- function(t, theta) {
  m <- length(t)
  res <- matrix(0, ncol = 2 * m, nrow = 2 * m)
  tiPlustj <- crossSum(t, t)
  tiMenostj <- crossSum(t,-t)
  phi <- ComplexCF_GTS(t, theta)
  phi_tiPlus_tj <- sapply(tiPlustj, ComplexCF_GTS, theta)
  phi_tiMenos_tj <- sapply(tiMenostj, ComplexCF_GTS, theta)
  res[1:m, 1:m] <-
...
DataVarRealCFMoment_GTS <- function(t, theta, x) {
  gt <-
    DataMatrixRealCFMomentCondition_GTS(t = t, theta = theta, x = x)
  stats::var(gt)
}

DataMatrixRealCFMomentCondition_GTS <- function(t, theta, x) {
  x <- matrix(c(x), ncol = 1)
  x_comp <- x %*% matrix(t, nrow = 1)
  x_comp <- matrix(complex(imaginary = x_comp), ncol = length(t))
  emp_car <- exp(x_comp)
  the_car <- ComplexCF_GTS(t, theta)
...
ComputeInvKbyG_GTS <-
  function(K, G, alphaReg, regularization, eps) {
    ...
    eigenAnalysis <- getSingularValueDecomposition_GTS(K)
...
getSingularValueDecomposition_GTS <- function(Kn) {
  if (isSymmetric(Kn)) {
...
ComputeT_GTS <- function(tScheme, eps, ...) {
  args <- list(...)
  if (tScheme == "free") {
    checkFreeArgs(args)
    t <- args$t_free
