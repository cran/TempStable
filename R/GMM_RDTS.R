...
GMMParametersEstim_RDTS <-
  function(x,
    ...
    Estim <- switch(algo,
                    `2SGMM` = {
                      Compute2SGMMParametersEstim_RDTS(
                        x = x,
                        theta0 = theta0,
                        alphaReg = alphaReg,
                        regularization = regularization,
                    ...
                    CueGMM = {
                      ComputeCueGMMParametersEstim_RDTS(
                        x = x,
                        theta0 = theta0,
                        alphaReg = alphaReg,
                        regularization = regularization,
...
Compute2SGMMParametersEstim_RDTS <-
  function(x,
    ...
    ProvidedWeightingMatrix <-
      ComputeWeightingMatrix_RDTS(t, theta1, x, WeightingMatrix, ...)
    CurrentEstimOptInfo <-
      ComputeCurrentEstim_RDTS(
        t_scheme = t_scheme,
        theta0 = theta1,
...
ComputeCurrentEstim_RDTS <-
  function(t_scheme,
    ...
    optOutput <-
      stats::nlminb(
        start = theta0,
        objective = ComputeObjective_RDTS,
        gradient = NULL,
        hessian = NULL,
        t = t,
        x = x,
...
ComputeObjective_RDTS <-
  function(theta,
    ...
    gbar <- sampleRealCFMoment_RDTS(x = x, t = t, theta = theta)
    K1gbar <-
      ComputeInvKbyG_RDTS(
        K = K,
        G = gbar,
...
sampleRealCFMoment_RDTS <- function(x, t, theta) {
  ComplexRes <- sampleComplexCFMoment_RDTS(x, t, theta)
  return(c(Re(ComplexRes), Im(ComplexRes)))
}

sampleComplexCFMoment_RDTS <- function(x, t, theta) {
  ecf <- function(tt)
    mean(exp(complex(imaginary = tt * x)))
  phiXn <- sapply(t, ecf)
  phiTheta <- ComplexCF_RDTS(t, theta)
  return(phiXn - phiTheta)
}

ComplexCF_RDTS <- function(t, theta) {
  alpha <- theta[1]
  delta <- theta[2]
  lambdap <- theta[3]
  lambdam <- theta[4]
  mu <- theta[5]
  CheckParametersRange_RDTS(c(alpha, delta, lambdap, lambdam, mu))
  charRDTS(t, alpha, delta, lambdap, lambdam, mu)
}
...
ComputeWeightingMatrix_RDTS <-
  function(t, theta, x, WeightingMatrix, ...) {
    switch(
      WeightingMatrix,
      OptAsym = {
        K <- asymVarRealCFMoment_RDTS(t = t, theta = theta)
      },
      DataVar = {
        K <- DataVarRealCFMoment_RDTS(t = t, theta = theta, x = x)
      },
      Id = {
...
asymVarRealCFMoment_RDTS <- function(t, theta) {
  m <- length(t)
  res <- matrix(0, ncol = 2 * m, nrow = 2 * m)
  tiPlustj <- crossSum(t, t)
  tiMenostj <- crossSum(t,-t)
  phi <- ComplexCF_RDTS(t, theta)
  phi_tiPlus_tj <- sapply(tiPlustj, ComplexCF_RDTS, theta)
  phi_tiMenos_tj <- sapply(tiMenostj, ComplexCF_RDTS, theta)
  res[1:m, 1:m] <-
...
DataVarRealCFMoment_RDTS <- function(t, theta, x) {
  gt <-
    DataMatrixRealCFMomentCondition_RDTS(t = t, theta = theta, x = x)
  stats::var(gt)
}

DataMatrixRealCFMomentCondition_RDTS <- function(t, theta, x) {
  x <- matrix(c(x), ncol = 1)
  x_comp <- x %*% matrix(t, nrow = 1)
  x_comp <- matrix(complex(imaginary = x_comp), ncol = length(t))
  emp_car <- exp(x_comp)
  the_car <- ComplexCF_RDTS(t, theta)
...
ComputeInvKbyG_RDTS <-
  function(K, G, alphaReg, regularization, eps) {
    ...
    eigenAnalysis <- getSingularValueDecomposition_RDTS(K)
...
getSingularValueDecomposition_RDTS <- function(Kn) {
  if (isSymmetric(Kn)) {
...
ComputeT_RDTS <- function(tScheme, eps, ...) {
  args <- list(...)
  if (tScheme == "free") {
    checkFreeArgs(args)
    t <- args$t_free
    ...
    checkMainArgs(args)
    t <-
      ComputeEquallySpacedPoints_RDTS(tScheme = tScheme, eps = eps, ...)
  } else if ((tScheme == "uniformOpt") ||
             (tScheme == "ArithOpt") || (tScheme == "VarOpt")) {
    nb_t <- checkMainArgs(args)
    t <-
      ComputeOptimisedPoints_RDTS(tScheme = tScheme, eps = eps, ...)
  }
  t
}
...
ComputeEquallySpacedPoints_RDTS <-
  function(..., tScheme, eps, x, nb_t, Constrained = TRUE) {
    Bands <-
      Compute_tBands_RDTS(x = x,
...
Compute_tBands_RDTS <- function(x, Constrained, eps, ...) {
  args <- list(...)
  if (!Constrained) {
    ...
    if ((tScheme == "uniformOpt") || (tScheme == "ArithOpt")) {
      ...
      if (FastOptim)
        t <-
          stats::optim(
            par = t0,
            fn = ObjectiveFctToMinIn_t_RDTS,
            gr = NULL,
            ...,
            method = "Nelder-Mead"
          )$par
      else
        t <- stats::nlminb(start = t0, objective = ObjectiveFctToMinIn_t_RDTS, ...)
    }
    t
  }
...
ComputeApproxOptSpacing_RDTS <-
  function(..., tScheme, eps, nb_t, Constrained, An) {
    ...
    if (Constrained) {
      Bands <- Compute_tauBands_RDTS(tScheme, eps, nb_t, An)
      tauInfo <-
        stats::nlminb(
          start = tau0,
          objective = ObjectiveFctToMinIn_tau_RDTS,
          gradient = NULL,
          hessian = NULL,
          tScheme = tScheme,
          eps = eps,
...
Compute_tauBands_RDTS <- function(tScheme, eps, nb_t, An) {
  if (tScheme == "uniformOpt") {
    lower = eps
    upper = An / nb_t
  } else if (tScheme == "ArithOpt") {
...
ObjectiveFctToMinIn_t_RDTS <-
  function(t,
           ...
           fctToApply = InvDet) {
    W <-
      GMMasymptoticVarianceEstim_RDTS(
        ...,
        t = t,
        theta = theta,
        x = x,
...
GMMasymptoticVarianceEstim_RDTS <-
  function(...,
    ...
    B <- jacobianSampleRealCFMoment_RDTS(t, theta)
    fct <-
      function(G)
        ComputeInvKbyG_RDTS(
          K = K,
...
jacobianSampleRealCFMoment_RDTS <- function(t, theta) {
  jac <- jacobianSampleComplexCFMoment_RDTS(t, theta)
  apply(jac, 2, function(x)
    c(Re(x), Im(x)))
}

jacobianSampleComplexCFMoment_RDTS <- function(t, theta) {
  -jacobianComplexCF_RDTS(t, theta)
}

jacobianComplexCF_RDTS <- function(t, theta) {
  ComplexCFtoDeriv <- function(th)
    ComplexCF_RDTS(t, th)
  NumDeriv_jacobian_RDTS(ComplexCFtoDeriv, theta)
}

NumDeriv_jacobian_RDTS <-
