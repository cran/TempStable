##### main function#####
GMCParametersEstim_GTS <- function(x, algo = c("2SGMC", "ITGMC", "CueGMC"),
                                   ncond, alphaReg = 0.01,
                                   regularization =
    ...
    method <- getGMCmethodName_GTS(algo = algo, ncond = ncondfl,
                                   alphaReg = alphaReg,
                                   regularization = regularization,
                                   WeightingMatrix = WeightingMatrix, ...)
    Estim <- switch(algo, `2SGMC` = {
        Compute2SGMCParametersEstim_GTS(x = x, ncond = ncondfl, theta0 = theta0,
                                        alphaReg = alphaReg,
                                        regularization = regularization,
                                        WeightingMatrix = WeightingMatrix,
                                        eps = eps, ...)
    }, ITGMC = {
        ComputeITGMCParametersEstim_GTS(x = x, ncond = ncondfl, theta0 = theta0,
                                        alphaReg = alphaReg,
                                        regularization = regularization,
                                        WeightingMatrix = WeightingMatrix,
        ...
        ComputeCueGMCParametersEstim_GTS(x = x, ncond = ncondfl,
    ...
    if (PrintTime) {
        CallingFct <- paste("GTS", "GMCParametersEstim", algo, "ncond=",
                            ncondfl, sep = "_")
...
getGMCmethodName_GTS <- function(algo, ncond, alphaReg, regularization,
                                 WeightingMatrix, ...) {
    args <- list(...)
    paste(algo,
...
##### GMC methods#####
Compute2SGMCParametersEstim_GTS <- function(x, ncond, theta0, alphaReg,
                                            regularization, WeightingMatrix,
                                            eps, ...) {
    iter = 0
    AllCurrentEstim <- ComputeCurrentGMC_GTS(theta0 = theta0, x = x,
                                             ncond = ncond, alphaReg = alphaReg,
                                             regularization = regularization,
                                             WeightingMatrix = "Id",
                                             eps = eps, ...)
    theta1 <- (AllCurrentEstim$OptInfo)$par
    ProvidedWeightingMatrix <- ComputeGMCWeightingMatrix_GTS(theta1, x, ncond,
                                                             WeightingMatrix,
                                                             ...)
    CurrentEstimOptInfo <-
      ComputeCurrentGMC_GTS(theta0 = theta1, x = x, ncond = ncond,
                            alphaReg = alphaReg,
                            regularization = regularization,
...
ComputeITGMCParametersEstim_GTS <- function(x, ncond, theta0, alphaReg,
                                            regularization, WeightingMatrix,
                                            IterationControl,
    eps, ...) {
    iter = 0
...
ComputeCueGMCParametersEstim_GTS <-
  function(x,
           ncond,
           theta0,
           alphaReg,
...
ComputeCurrentGMC_GTS <-
  function(theta0,
    ...
    optOutput <-
      stats::nlminb(
        start = theta0,
        objective = ComputeGMCObjective_GTS,
        gradient = NULL,
        hessian = NULL,
        x = x,
        ncond = ncond,
...
ComputeGMCObjective_GTS <-
  function(theta,
    ...
    gbar <- sampleCumulants_GTS(x = x, ncond = ncond, theta = theta)
    W1gbar <-
      ComputeInvKbyG_GTS(
        K = W,
        G = gbar,
...
sampleCumulants_GTS <- function(x, ncond, theta) {
  sampleCumulants <-
    empiricalCumulants_GTS(x = x, ncond = ncond) -
    theoreticalCumulants_GTS(ncond = ncond, theta = theta)
  return(sampleCumulants)
}

empiricalCumulants_GTS <- function(x, ncond) {
  CumFinder_GTS(x = x, jmax = ncond)
}
...
theoreticalCumulants_GTS <- function(ncond, theta) {
  ...
  X <- 1:ncond
  theoretical <-
    sapply(X = X, jththeoreticalCumulant_GTS, theta = theta)
  return(theoretical)
}
...
ComputeGMCWeightingMatrix_GTS <-
  function(theta, x, ncond, WeightingMatrix, ...) {
    switch(
      WeightingMatrix,
      OptAsym = {
        W <- asymVarCumulants_GTS(theta = theta,
                                  x = x,
                                  ncond = ncond)
      },
      Id = {
...
asymVarCumulants_GTS <- function(theta, x, ncond) {
  k.pow <- (1:(ncond - 1))
  cumus <- CumFinder_GTS(x = x, jmax = ncond)[1:(ncond - 1)]
  g <- function(n, z) {
  ...
  gvec <- function(z) {
    sapply(X = (1:ncond),
           FUN = g,
           z = z) - theoreticalCumulants_GTS(ncond = ncond, theta = theta)
  }
  gmat <- function(z) {
