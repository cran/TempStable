...
GMMParametersEstim_CTS <-
  function(x,
    ...
    Estim <- switch(algo,
                    `2SGMM` = {
                      Compute2SGMMParametersEstim_CTS(
                        x = x,
                        theta0 = theta0,
                        alphaReg = alphaReg,
                        regularization = regularization,
                    ...
                    ITGMM = {
                      ComputeITGMMParametersEstim_CTS(
                        x = x,
                        theta0 = theta0,
                        alphaReg = alphaReg,
                        regularization = regularization,
                    ...
                    CueGMM = {
                      ComputeCueGMMParametersEstim_CTS(
                        x = x,
                        theta0 = theta0,
                        alphaReg = alphaReg,
                        regularization = regularization,
    ...
    if (PrintTime) {
      CallingFct <-
        paste("Classic", "GMMParametersEstim", algo, t_scheme, sep = "_")
...
Compute2SGMMParametersEstim_CTS <-
  function(x,
    ...
    ProvidedWeightingMatrix <-
      ComputeWeightingMatrix_CTS(t, theta1, x, WeightingMatrix, ...)
    CurrentEstimOptInfo <-
      ComputeCurrentEstim_CTS(
        t_scheme = t_scheme,
        theta0 = theta1,
...
ComputeCurrentEstim_CTS <-
  function(t_scheme,
    ...
    optOutput <-
      stats::nlminb(
        start = theta0,
        objective = ComputeObjective_CTS,
        gradient = NULL,
        hessian = NULL,
        t = t,
        x = x,
...
DataVarRealCFMoment_CTS <- function(t, theta, x) {
  gt <-
    DataMatrixRealCFMomentCondition_CTS(t = t, theta = theta, x = x)
  stats::var(gt)
}
...
ComputeT_CTS <- function(tScheme, eps, ...) {
  ...
  if (tScheme == "free") {
    ...
    checkMainArgs(args)
    t <-
      ComputeEquallySpacedPoints_CTS(tScheme = tScheme, eps = eps, ...)
  } else if ((tScheme == "uniformOpt") ||
             (tScheme == "ArithOpt") || (tScheme == "VarOpt")) {
    nb_t <- checkMainArgs(args)
    t <-
      ComputeOptimisedPoints_CTS(tScheme = tScheme, eps = eps, ...)
  }
  t
}
...
ComputeEquallySpacedPoints_CTS <-
  function(..., tScheme, eps, x, nb_t, Constrained = TRUE) {
    Bands <-
      Compute_tBands_CTS(x = x,
    ...
    if ((tScheme == "uniformOpt") || (tScheme == "ArithOpt")) {
      ...
      if (FastOptim)
        t <-
          stats::optim(
            par = t0,
            fn = ObjectiveFctToMinIn_t_CTS,
            gr = NULL,
            ...,
            method = "Nelder-Mead"
          )$par
      else
        t <- stats::nlminb(start = t0, objective = ObjectiveFctToMinIn_t_CTS, ...)
    }
    t
  }
  ...
  function(..., tScheme, eps, nb_t, Constrained, An) {
    ...
    if (Constrained) {
      ...
      tauInfo <-
        stats::nlminb(
          start = tau0,
          objective = ObjectiveFctToMinIn_tau_CTS,
          gradient = NULL,
          hessian = NULL,
          tScheme = tScheme,
          eps = eps,
...
ObjectiveFctToMinIn_t_CTS <-
  function(t,
           ...
           fctToApply = InvDet) {
    W <-
      GMMasymptoticVarianceEstim_CTS(
        ...,
        t = t,
        theta = theta,
        x = x,
